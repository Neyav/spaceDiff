...
namespace spaceDiff
{
    ...
    internal class spaceDiff
    {
        ...
        public string getNextBlock(ref int aPos, ref bool isMatch, ref bool fileDone, bool isOldFile)
        {
            string output = "";
            ...
            { 
                foreach (dualRange dualRange in masterRangeList)
                {   // Find the closest range match.
                    if (dualRange.oldRange.start > aPos)
                    {
                        isMatch = false;

                        // Copy the data to output string until we hit the start of the range.
                        for (int i = aPos; i < dualRange.oldRange.start; i++)
                        {
                            output += (char)oldFile[i];
                        }
                        aPos = dualRange.oldRange.start;

                        return output;
                    }
                    else if (dualRange.oldRange.start <= aPos && dualRange.oldRange.end >= aPos)
                    {
                        isMatch = true;

                        // Copy the data to output string until we hit the start of the range.
                        for (int i = aPos; i <= dualRange.oldRange.end; i++)
                        {
                            output += (char)oldFile[i];
                        }
                        aPos = dualRange.oldRange.end + 1;

                        if (aPos >= oldFile.Length)
                            fileDone = true;

                        return output;
                    }
                }
                // Dump the rest of the file into the output string and we're done.
                for (int i = aPos; i < oldFile.Length; i++)
                {
                    output += (char)oldFile[i];
                }
                aPos = oldFile.Length;
            ...
            else
            {
                foreach (dualRange dualRange in masterRangeList)
                {   // Find the closest range match.
                    if (dualRange.newRange.start > aPos)
                    {
                        isMatch = false;

                        // Copy the data to output string until we hit the start of the range.
                        for (int i = aPos; i < dualRange.newRange.start; i++)
                        {
                            output += (char)newFile[i];
                        }
                        aPos = dualRange.newRange.start;
                        return output;
                    }
                    else if (dualRange.newRange.start <= aPos && dualRange.newRange.end >= aPos)
                    {
                        isMatch = true;

                        // Copy the data to output string until we hit the start of the range.
                        for (int i = aPos; i <= dualRange.newRange.end; i++)
                        {
                            output += (char)newFile[i];
                        }
                        aPos = dualRange.newRange.end + 1;

                        if (aPos >= newFile.Length)
                            fileDone = true;
                ...
                // Dump the rest of the file into the output string and we're done.
                for (int i = aPos; i < newFile.Length; i++)
                {
                    output += (char)newFile[i];
                }
                aPos = newFile.Length;
